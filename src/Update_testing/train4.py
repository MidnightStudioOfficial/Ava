...
if __name__ == '__main__':
    ...
    for patience_value in early_stopping_patience_values:
        ...
        early_stopping = EarlyStopping(monitor='val_accuracy', patience=patience_value, restore_best_weights=True) #monitor='val_loss'
        history = model.fit(
        ...
        for s in testing:
            ...
            print(s, intent[0])
            test_results[patience_value].append((s, intent[0]))

        # Compare with the best result so far (based on both validation accuracy and test results)
        if val_accuracy > best_val_accuracy and test_results[patience_value] == test_results[best_experiment_params.get('patience', patience_value)]:
