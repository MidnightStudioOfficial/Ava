...
class SSetup(Toplevel):
    def __init__(self: Toplevel, parent: Tk, settings: dict) -> None:
        ...
        # configure window
        self.geometry(
            f'500x620+{int(self.winfo_x() + ((self.winfo_screenwidth() / 2) - 250))}+{int(self.winfo_y() + ((self.winfo_screenheight() / 2) - 310))}')
    ...
    def init_ui(self: Toplevel) -> None:
        ...
        self.progress_bar = ttk.Progressbar(
            self, maximum=self.pages * 100, value=0)
        ...
        ttk.Label(welcome_content, text='Welcome to Sounder5!',
                  style='second.TLabel').pack(side='top', pady=10, anchor='center')
        ttk.Button(welcome_content, text='Start listening!', image=self.icons['arrow'][1], compound='right', style='second.TButton', command=lambda: self.next_page(
            import_panel)).pack(side='top', pady=10, anchor='center')
        ...
        ttk.Label(import_panel, text='Import settings!', style='second.TLabel').pack(
            side='top', anchor='center', pady=(10, 0))
        ttk.Label(import_panel, text='Import old settings or set up as new!',
                  style='third.TLabel').pack(side='top', anchor='center')
        import_content: ttk.Frame = ttk.Frame(import_panel)
        ttk.Button(import_content, text='Set up as new', image=self.icons['arrow'][1], compound='right', style='second.TButton', command=lambda: self.next_page(
            folder_panel)).pack(side='bottom', pady=10, anchor='center', fill='x')
        ttk.Button(import_content, text='Choose a settings file', image=self.icons['plus'], compound='right', style='second.TButton', command=self.import_settings).pack(
            side='bottom', pady=10, anchor='center', fill='x')
        ...
        ttk.Label(folder_panel, text='Let\'s start with music!', style='second.TLabel').pack(
            side='top', anchor='center', pady=(10, 0))
        ttk.Label(folder_panel, text='Show us where you store your music.',
                  style='third.TLabel').pack(side='top', anchor='center')
        # player content
        folder_canvas: Canvas = Canvas(
            folder_panel, background=self['background'], bd=0, highlightthickness=0, yscrollcommand=folder_scrollbar.set, takefocus=False)
        # link scrollbar to canvas
        folder_scrollbar.configure(command=folder_canvas.yview)
        self.folder_panels: ttk.Frame = ttk.Frame(folder_canvas)
        self.folder_panels.bind('<Configure>', lambda _: folder_canvas.configure(
            scrollregion=folder_canvas.bbox("all")))
        folder_window = folder_canvas.create_window(
            (0, 0), window=self.folder_panels, anchor='nw')
        folder_canvas.bind('<Configure>', lambda _: folder_canvas.itemconfigure(
            folder_window, width=folder_canvas.winfo_width(), height=0))
        folder_canvas.pack(side='top', padx=10,  pady=10,
                           anchor='center', fill='both')
        folder_buttons: ttk.Frame = ttk.Frame(folder_panel)
        # add folder button
        ttk.Button(folder_buttons, text='Add folder', image=self.icons['plus'], compound='left',
                   style='second.TButton', command=self.add_folder).pack(side='left', pady=5, padx=(10, 25))
        # skip / continue button
        self.folder_button = ttk.Button(
            folder_buttons, text='Skip', image=self.icons['arrow'][1], compound='right', style='second.TButton', command=lambda: self.next_page(appearance_panel))
        ...
        ttk.Label(appearance_panel, text='Theme!', style='second.TLabel').pack(
            side='top', anchor='center', pady=(10, 0))
        ttk.Label(appearance_panel, text='Shine bright like a star or be dark like a boss!',
                  style='third.TLabel').pack(side='top', anchor='center')
        appearance_content: ttk.Frame = ttk.Frame(appearance_panel)
        ttk.Radiobutton(appearance_content, image=self.icons['brush'], text='Light', compound='left', variable=self.theme,
                        value='Light', command=self.change_theme).pack(side='top', fill='x', padx=10, pady=5, ipadx=45)
        ttk.Radiobutton(appearance_content, image=self.icons['brush'], text='Dark', compound='left', variable=self.theme,
                        value='Dark', command=self.change_theme).pack(side='top', fill='x', padx=10, pady=5, ipadx=45)
        ttk.Radiobutton(appearance_content, image=self.icons['brush'], text='System', compound='left', variable=self.theme,
                        value='System', command=self.change_theme).pack(side='top', fill='x', padx=10, pady=5, ipadx=45)
        ttk.Radiobutton(appearance_content, image=self.icons['brush'], text='Contrast', compound='left', variable=self.theme,
                        value='Contrast', command=self.change_theme).pack(side='top', fill='x', padx=10, pady=5, ipadx=45)
        appearance_content.pack(anchor='center', padx=10, pady=10, expand=True)
        ttk.Button(appearance_panel, text='Next', image=self.icons['arrow'][1], compound='right', style='second.TButton', command=lambda: self.next_page(
            updates_panel)).pack(side='bottom', pady=10, anchor='center')
        appearance_panel.place(x=0, y=0, relwidth=1, relheight=1)
        # page 5
        updates_panel: ttk.Frame = ttk.Frame(pages_panel)
        ttk.Label(updates_panel, text='Updates!', style='second.TLabel').pack(
            side='top', anchor='center', pady=(10, 0))
        ttk.Label(updates_panel, text='That\'s right, everybody likes updates!',
                  style='third.TLabel').pack(side='top', anchor='center')
        updates_content: ttk.Frame = ttk.Frame(updates_panel)
        ttk.Radiobutton(updates_content, image=self.icons['checkmark'], text='Yes, check for updates!', compound='left',
                        value=True, variable=self.updates, command=self.change_updates).pack(side='top', fill='x', padx=10, pady=5, ipadx=45)
        ttk.Radiobutton(updates_content, image=self.icons['delete'], text='No :(', compound='left', value=False,
                        variable=self.updates, command=self.change_updates).pack(side='top', fill='x', padx=10, pady=5, ipadx=45)
        updates_content.pack(anchor='center', padx=10, pady=10, expand=True)
        ttk.Button(updates_panel, text='Next', image=self.icons['arrow'][1], compound='right', style='second.TButton', command=lambda: self.next_page(
            self.final_panel)).pack(side='bottom', pady=10, anchor='center')
        ...
        ttk.Label(final_content, text='That\'s all!', style='second.TLabel').pack(
            side='top', pady=10, anchor='center')
        ttk.Button(final_content, text='Finish', image=self.icons['checkmark'], compound='left',
                   style='second.TButton', command=self.exit_app).pack(side='top', pady=10, anchor='center')
    ...
    def apply_theme(self: Toplevel) -> None:
        ...
        # label
        self.layout.configure('TLabel', background='#111', relief='flat', font=(
            'catamaran 13 bold'), foreground='#fff')
        self.layout.configure('second.TLabel', background='#212121', font=(
            'catamaran 30 bold'), anchor='center')
        self.layout.configure('third.TLabel', background='#212121')
        # radiobutton
        self.layout.configure('TRadiobutton', background='#212121', relief='flat', font=(
            'catamaran 13 bold'), foreground='#fff', anchor='w', padding=10, width=12)
        self.layout.map('TRadiobutton', background=[
                        ('pressed', '!disabled', '#111'), ('active', '#111'), ('selected', '#111')])
        # button
        self.layout.configure('TButton', background='#111', relief='flat', font=(
            'catamaran 13 bold'), foreground='#fff', anchor='w')
    ...
    def add_folder(self: Toplevel) -> None:
        ...
        if temp_dir and not temp_dir in self.settings['folders']:
            ...
            # draw folder in app
            folder_panel: ttk.Frame = ttk.Frame(
                self.folder_panels, style='second.TFrame')
            path_label = ttk.Label(folder_panel, image=self.icons['folder'], text=basename(
                temp_dir), compound='left')
            path_label.pack(side='left', anchor='center',
                            fill='y', pady=10, padx=10)
            ttk.Button(folder_panel, image=self.icons['delete'], takefocus=False, command=lambda: self.remove_folder(
                folder_panel, temp_dir)).pack(side='right', padx=10, anchor='center')
            folder_panel.bind('<Leave>', lambda _: path_label.configure(
                text=basename(temp_dir)))
            folder_panel.bind(
                '<Enter>', lambda _: path_label.configure(text=temp_dir))
        ...
        if settings_file:
            with open(settings_file, 'r') as data:
                try:
                    ...
                    Thread(target=self.animate_progress,
                           args=(400,), daemon=True).start()
