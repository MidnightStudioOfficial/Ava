...
def generate_response(prompt):
    ...
    while True:
        input_sequence = np.array([list(vocabulary).index(word) for word in generated_words if word in vocabulary])
        if len(input_sequence) == 0:
            generated_response = "I'm sorry, I couldn't understand your prompt."
            break
        ...
        output_probs = model.predict(input_sequence)[0]
        output_probs /= output_probs.sum()  # Normalize probabilities
        output_probs_flat = output_probs.flatten()
        next_word_index = np.random.choice(np.arange(len(output_probs_flat)), p=output_probs_flat)
