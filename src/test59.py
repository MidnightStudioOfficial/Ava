...
num_samples = 2
time_of_day = np.array([0.1, 0.3])  # Random time values between 0 and 1
personality_traits = np.array([[0.2, 0.3, 0.78768, 0.5, 0.6]])  # Random personality traits as a 2D array
sentiment_score = np.array([0.2, 0.4])  # Random sentiment scores between 0 and 1
mood = np.array([0.3, 0.5])  # Random mood values between 0 and 1
...
inputs = torch.cat((time_of_day, personality_traits, sentiment_score), dim=0) #, dim=1
