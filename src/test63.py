...
alpha = 0.12  # Smoothing factor 0.1
previous_wave_data = None

# Function to smooth the wave data using a low-pass filter
def smooth_wave_data(wave_data):
    global previous_wave_data
    if previous_wave_data is None:
        previous_wave_data = wave_data
    else:
        wave_data = alpha * wave_data + (1 - alpha) * previous_wave_data
        previous_wave_data = wave_data
    return wave_data
...
def update_canvas(stream):
    try:
        ...
        wave_data = smooth_wave_data(wave_data)  # Apply low-pass filter
