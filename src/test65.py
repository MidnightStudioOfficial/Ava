import tkinter as tk
import pyaudio
import numpy as np
import random

# Constants for audio processing
FORMAT = pyaudio.paInt16
CHANNELS = 2  # Number of audio channels
RATE = 44100
CHUNK = 1024

# Create the Tkinter application window
window = tk.Tk()
window.title("Multi-Frequency Waves")
canvas = tk.Canvas(window, width=800, height=400, bg="black")
canvas.pack()

# Function to generate random color
def random_color():
    r = random.randint(0, 255)
    g = random.randint(0, 255)
    b = random.randint(0, 255)
    return f"#{r:02x}{g:02x}{b:02x}"

# Generate colors for each channel
wave_colors = [random_color() for _ in range(CHANNELS)]

# Low-pass filter parameters
cutoff_freq = 1000  # Cutoff frequency for the low-pass filter
prev_filtered_data = None

# Function to apply a low-pass filter to the wave data
def apply_lowpass_filter(wave_data):
    global prev_filtered_data
    if prev_filtered_data is None:
        prev_filtered_data = wave_data
    else:
        # Apply the low-pass filter using IIR filtering
        filtered_data = prev_filtered_data + (cutoff_freq / RATE) * (wave_data - prev_filtered_data)
        prev_filtered_data = filtered_data
    return filtered_data

# Function to draw the voice waves on the canvas
def draw_waves(wave_data):
    canvas.delete("wave")  # Clear previous waves
    num_samples = len(wave_data[0])
    x_scale = 800 / num_samples
    x = 0
    for i, wave_channel in enumerate(wave_data):
        for channel in range(CHANNELS):
            points = []
            fake_points = []  # Store points for the fake wave line
            for j in range(0, num_samples):
                y = 200 + wave_channel[j][channel] * 100 * (1 + i) * (1 + np.sin(x * 0.01))  # Adjust amplitude and vertical position with real-time animation
                fake_y = 200 + wave_channel[max(0, j-46)][channel] * 100 * (1 + i) * (1 + np.sin(x * 0.01))  # Adjust amplitude and vertical position for the fake line (10 steps behind)
                points.extend([x, y])
                fake_points.extend([x, fake_y])
                x += x_scale
            color = wave_colors[channel]  # Assign a color to each channel
            canvas.create_line(points, fill=color, width=2, tags="wave")
            canvas.create_line(fake_points, fill="gray", width=1, tags="wave")

# Function to read audio data and update the canvas
def update_canvas(stream):
    try:
        data = stream.read(CHUNK, exception_on_overflow=False)
        wave_data = np.frombuffer(data, dtype=np.int16).reshape(-1, CHUNK, CHANNELS) / 32768.0  # Normalize wave data
        filtered_data = apply_lowpass_filter(wave_data)  # Apply low-pass filter
        draw_waves(filtered_data)
    except Exception as e:
        print("Error:", str(e))

# Set up PyAudio
audio = pyaudio.PyAudio()
stream = audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, frames_per_buffer=CHUNK)

# Start the Tkinter event loop
def update():
    update_canvas(stream)
    window.after(10, update)  # Increase the speed for a more active visualization

update()  # Start the update loop
window.mainloop()

# Clean up
stream.stop_stream()
stream.close()
audio.terminate()
