...
CHANNELS = 1  # Number of audio channels 2
RATE = 44100
CHUNK = 1094 #1024
...
cutoff_freq = 900  # Cutoff frequency for the low-pass filter 1000
prev_filtered_data = None

# Function to apply a low-pass filter to the wave data
def apply_lowpass_filter(wave_data):
    global prev_filtered_data
    if prev_filtered_data is None:
        prev_filtered_data = wave_data
    else:
        # Apply the low-pass filter using IIR filtering
        filtered_data = prev_filtered_data + (cutoff_freq / RATE) * (wave_data - prev_filtered_data)
    ...
    for i, wave_channel in enumerate(wave_data):
        for channel in range(CHANNELS):
            ...
            for j in range(0, num_samples):
                y = 200 + wave_channel[j][channel] * 100 * (1 + i) * (1 + np.sin(x * 0.01))  # Adjust amplitude and vertical position with dynamic animation
                fake_y = 200 + wave_channel[max(0, j-46)][channel] * 100 * (1 + i) * (1 + np.sin(x * 0.01))  # Adjust amplitude and vertical position for the fake line (10 steps behind)
                points.extend([x, y])
...
def update_canvas(stream):
    try:
        ...
        filtered_data = apply_lowpass_filter(wave_data)  # Apply low-pass filter
        draw_waves(filtered_data)
...
def update():
    ...
    window.update()  # Update the window to provide a more responsive animation
    window.after(3, update)  # Increase the speed for a more active visualization 10
