...
class MeditationApp:
    ...
    def start_exercise(self):
        ...
        self.start_visualization(duration_seconds)  # Start the breathing visualization
    ...
    def start_visualization(self, duration):
        breath_cycles = 20  # Number of breath cycles for the visualization 10
        inhale_time = duration // (2 * breath_cycles)
        exhale_time = inhale_time

        self.animate_breathing(self.canvas, inhale_time, True)
        self.root.after(inhale_time * 1000, self.animate_breathing, self.canvas, exhale_time, False)
        self.root.after(duration * 1000, self.reset_visualization, self.canvas)

    def animate_breathing(self, canvas, duration, inhale):
        if inhale:
            color = "blue"
            text = "Inhale"
        else:
        ...
        canvas.delete("all")
        canvas.create_oval(50, 50, 150, 150, outline=color, width=2)
        canvas.create_text(100, 100, text=text, fill=color)
